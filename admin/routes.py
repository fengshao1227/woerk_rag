...
@router.get("/my-api-keys/{key_id}/download")
async def download_mcp_server(
    ...
    返回一个 ZIP 文件，包含：
    - mcp_server/server.py (MCP 服务器代码)
    - mcp_server/requirements.txt (依赖)
    - README.md (使用说明)
    ...
    if not api_key:
        raise HTTPException(status_code=404, detail="卡密不存在或无权访问")

    # 读取 MCP 服务器代码
    mcp_server_path = Path(__file__).parent.parent / "mcp_server" / "server.py"
    if not mcp_server_path.exists():
        raise HTTPException(status_code=500, detail="MCP 服务器文件不存在")

    mcp_server_code = mcp_server_path.read_text(encoding="utf-8")

    # 生成 requirements.txt
    requirements = """# RAG MCP Server 依赖
httpx>=0.25.0
mcp>=1.0.0
"""

    # 生成 README
    readme = f"""# RAG 知识库 MCP Server
...
{{
  "mcpServers": {{
    "rag-knowledge": {{
      "command": "python",
      "args": ["<MCP服务器路径>/server.py"],
      "env": {{
        "RAG_API_KEY": "{api_key.key}"
      }}
    }}
  }}
}}
...
> ⚠️ 请将 `<MCP服务器路径>` 替换为实际的绝对路径

### 3. 重启 Claude Desktop

配置完成后重启 Claude Desktop，即可在对话中使用知识库功能。

## 功能说明

- **query**: 基于知识库的智能问答
- **search**: 向量检索相关内容
- **add_knowledge**: 添加新知识到知识库

## 权限说明

此卡密绑定用户: **{current_user.username}**
...
'''
    ...
    with zipfile.ZipFile(zip_buffer, 'w', zipfile.ZIP_DEFLATED) as zf:
        zf.writestr("mcp_server/server.py", mcp_server_code)
        zf.writestr("mcp_server/requirements.txt", requirements)
        zf.writestr("README.md", readme)
        zf.writestr("claude_desktop_config.json", claude_config)

    zip_buffer.seek(0)

    # 生成文件名
    safe_name = api_key.name.replace(" ", "_").replace("/", "_")[:20]
    filename = f"rag_mcp_server_{safe_name}.zip"
